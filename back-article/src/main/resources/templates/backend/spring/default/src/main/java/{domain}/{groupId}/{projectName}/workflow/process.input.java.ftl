...
import ${config.domain}.${config.groupId}.${config.projectName}.ws.dto.${dependency.subModule.folder}.${dependency.name}Dto;
            <#else>
...
import ${config.domain}.${config.groupId}.${config.projectName}.zynerator.process.AbstractProcessInput;


@JsonInclude(JsonInclude.Include.NON_NULL)
public class ${pojo.name}${process.name?cap_first}${role.name?cap_first}Input extends AbstractProcessInput {
